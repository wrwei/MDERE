...
The users of SACM may also organise \textit{ExpressionElement} into \textit{Categories}. For example, the users may create an \textit{Category} of Hazard, and associate the \textit{Term} H1 to it. \will{may need to talk about what origin means}
...
%\begin{figure}
%	\centering
%	\includegraphics[width=1\linewidth]{fig/assertedClaim.eps}
%	\caption{Packages of SACM}
%	\label{fig:assertedClaim}
%\end{figure}

Figure~\ref{fig:assertedClaim} provides an example on how to construct a \textit{Claim}. A \textit{Claim} can have a name and a description, captured by \textit{LangString} and \textit{Description} respectively. A \textit{Claim} is \textbf{asserted} if it supported by other \textit{Claim}s. In this example, it is connected by an \textit{AssertedInference} with another \textit{Claim} (details ommitted). The equivalence of \textit{Claim} C1 in Figure~\ref{fig:assertedClaim} in GSN is provided in Figure~\ref{fig:goalingsn}, where the \textit{Goal} G1 bears the same semantics with C1.

%\begin{figure}
%	\centering
%	\includegraphics[width=0.3\linewidth]{fig/goal_asserted.pdf}
%	\caption{Packages of SACM}
%	\label{fig:arg}
%\end{figure}
%\begin{figure}
...
%		\centering
%		\includegraphics[width=1\linewidth]{fig/goal_asserted.png}
%		\caption{A \textit{Goal} in GSN.}
%		\label{fig:goalingsn}
%	\end{subfigure}
%	\caption{A \textit{Claim} in SACM and a \textit{Goal} in GSN.}
%	\label{fig:claimandgoal}
%\end{figure}
