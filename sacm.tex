...
\begin{itemize}
	\item \textbf{AssertedCotnext} - this relationship is used to connect contextual \textit{Assertion}s to an \textit{Assertion};
...
Since GSN notations are widely accepted and understood, we compare GSN notations with their equivalent model elements in SACM to illustrate how SACM elements can be used to denote the same meaning carried by their GSN counterparts.
...
\begin{figure}
	\includegraphics[width=.99\linewidth]{pattern.eps}
...
The top level \textit{Claim} of this pattern is \textbf{SysProps}, which maps to the \textit{Goal} \textbf{Goal:sysProps} in Figure~\ref{fig:dmilsPattern}. Note that \textbf{SysProps}'s \textit{+isAbstract} is set to \textit{true} since this is an abstract \textit{Claim} (a template). Now we focus on the \textit{description} of \textbf{SysProps}, within which an \textit{ExpressionLangString} is used. The \textit{ExpressionLangString} refers to the \textit{Expression} in \textit{TerminologyPackage} TP1 (which contains value \textit{\{D-MILS System\}} required properties are enforced). The curly brackets are a convention in GSN, which are called \textit{role}s in GSN. \textit{Role}s are essentially place holders in the pattern, the contents enclosed in the curly brackets will be replaced by actual system information if the pattern gets instantiated. In this case, when the pattern is instantiated, \{D-MILS System\} will be replaced with the actual system name (we will discuss instantiation rules later). 

In the \textit{TerminologyPackage} TP1, the \textit{Expression} refers to the \textit{Term} \textit{D-MILS System}. This is a typical use of \textit{Term} in structured argumentation. When the pattern is \textit{instantiated}, the \textit{Term} should have its \textit{+externalReference} feature populated to import actual system information. 

The structure of the argumentation in Figure~\ref{fig:sacmPattern} follows that in Figure~\ref{fig:dmilsPattern}, where the \textit{Claim} \textbf{PropEnforced} maps to \textit{Goal} \textbf{Goal:propEnforced}, the \textit{ArgumentReasoning} \textbf{sysSecurity} maps to \textit{Strategy} \textbf{Strat:sysSecurity}. Note that the \textit{Claim}s in yellow colour are the templates, which have their \textit{+isAbstract} feature set to true. 

GSN pattern instantiation is often a manual procedure as safety case developers need to comprehend the GSN pattern and replace the \textit{role}s in the pattern with actual system information. In \cite{hawkins2015weaving}, a model-based approach is proposed, which makes use of a \textit{weaving model} to link the elements in the GSN with elements in system models. This is typically due to the fact that in GSN there are no means to specify instantiation rules for the GSN patterns. In SACM, as previously mentioned in Section~\ref{sec:basePack}, the element \textit{ImplmentationConstraint} can be used to specify the instantiation rules for patterns. This is illustrated in Figure~\ref{fig:sacmPattern} （element in blue）, that an \textit{ImplementationConstraint} is associated to the \textit{Term} \textit{D-MILS System}. Due to the space limit, we omit the containing elements of the \textit{ImplementationConstraint}. In this example, we use \textit{OCL} to describe the instantiation rule, in the fact that the \textit{Term} \textit{D-MILS System} will be replaced by the execution result of the OCL statement: \textit{System.allInstances()$\rightarrow$first().name} 

Evidently, pattern instantiation needs tool support and model query engine to execute. The language used in \textit{ImplementationConstraint} are not limited to OCL, or even computer languages. \textit{ImplmentationConstraint}s described in natural languages can also be used as instantiation rules, except that the instantiation procedure is limited to manual only. 

When a SACM model instantiates a SACM pattern, it can relate to the SACM pattern via the \textit{+abstractForm} feature. For example, if a \textit{Claim} C1 created by instantiating the template \textbf{SysProps}, it can refer to \textbf{SysProps} via the \textit{+abstractForm} for future reference. 
