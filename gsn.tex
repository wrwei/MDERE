...
\begin{figure}[ht!]
	\centering
	\includegraphics[width=.7\linewidth]{pattern_example.pdf}
...
\{System X\} in \underline{G1} will be replaced with the actual name of the system when the pattern is instantiated.
Similarly, the \textit{SupportedBy} connector between \underline{S1} and \underline{G2} specifies that when the pattern is instantiated, there should be $n$ (the number of safety-related functions implemented by the system) \textit{Goal}s and $n$ \textit{SupportedBy} connected to \underline{S1}. 
